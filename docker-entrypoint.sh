	...
	for f; do
		case "$f" in
			...
			*.sql.gz)	printf '%s: running %s\n' "$0" "$f"; 
						...
						database=${file%.*}
						export DBNAME=$database
						gunzip -c "$f" | docker_process_sql; printf '\n' ;;
			*.sql.xz)  	printf '%s: running %s\n' "$0" "$f"; 
						file=${f##*/}
						database=${file%.*}
						export DBNAME=$database
						xzcat "$f" | docker_process_sql; printf '\n' ;;
			*.sql.zst) 	printf '%s: running %s\n' "$0" "$f"; 
						file=${f##*/}
						database=${file%.*}
						export DBNAME=$database
						zstd -dc "$f" | docker_process_sql; printf '\n' ;;
			*)         printf '%s: ignoring %s\n' "$0" "$f" ;;
		esac
		printf '\n'
	done
...
docker_setup_db() {
	...
	dbAlreadyExists="$(
		POSTGRES_DB_1= docker_process_sql --dbname postgres --set db="$POSTGRES_DB_1" --tuples-only <<-'EOSQL'
			SELECT 1 FROM pg_database WHERE datname = :'db' ;
		EOSQL
	)"
	if [ -z "$dbAlreadyExists" ]; then
